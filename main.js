...
// Share buttons
        ...
        if (shareInsta) {
            shareInsta.addEventListener('click', () => this.shareStory('facebook'));
        }
        if (shareWa) {
...
>
 ...
 <button id="share-wa" class="w-full flex items-center justify-center gap-2 bg-green-500 text-white rounded-lg py-2 font-semibold hover:bg-green-600 transition"><svg class='w-5 h-5' fill='currentColor' viewBox='0 0 24 24'><path d='M20.52 3.48A11.93 11.93 0 0 0 12 0C5.37 0 0 5.37 0 12c0 2.11.55 4.16 1.6 5.97L0 24l6.22-1.63A11.93 11.93 0 0 0 12 24c6.63 0 12-5.37 12-12 0-3.19-1.24-6.19-3.48-8.52zM12 22c-1.85 0-3.68-.5-5.25-1.44l-.37-.22-3.69.97.99-3.59-.24-.37A9.94 9.94 0 0 1 2 12c0-5.52 4.48-10 10-10s10 4.48 10 10-4.48 10-10 10zm5.2-7.6c-.28-.14-1.65-.81-1.9-.9-.25-.09-.43-.14-.61.14-.18.28-.7.9-.86 1.08-.16.18-.32.2-.6.07-.28-.14-1.18-.44-2.25-1.4-.83-.74-1.39-1.65-1.55-1.93-.16-.28-.02-.43.12-.57.13-.13.28-.34.42-.51.14-.17.18-.29.28-.48.09-.19.05-.36-.02-.5-.07-.14-.61-1.47-.84-2.01-.22-.53-.45-.46-.62-.47-.16-.01-.36-.01-.56-.01-.19 0-.5.07-.76.34-.26.27-1 1-.97 2.43.03 1.43 1.04 2.81 1.19 3 .15.19 2.05 3.13 5.01 4.27.7.3 1.25.48 1.68.61.71.23 1.36.2 1.87.12.57-.09 1.65-.67 1.88-1.32.23-.65.23-1.2.16-1.32-.07-.12-.25-.19-.53-.33z'/></svg>شارك عبر واتساب</button>
        ...
        try {
            ...
            setTimeout(() => {
              ...
              copyBtn && copyBtn.addEventListener('click', () => {
                const storyText = this.storySteps && this.storySteps.length > 0 ? this.storySteps.join('\n\n') : '';
            ...
            //   });
              // تعريف متغيرات Cloudinary بشكل واضح في الأعلى
              
              shareInsta && shareInsta.addEventListener('click', async () => {
                ...
                // إخفاء كل العناصر ما عدا #full-story مؤقتاً
                const allEls = Array.from(document.body.children);
                const fullStory = document.getElementById('full-story');
                const hiddenEls = [];
                // إنشاء عنصر مؤقت يحتوي فقط نص القصة
                ...
                textDiv.innerHTML = Array.from(fullStory.querySelectorAll('span.block')).map(e => e.textContent).join('<br><br>');
                textDiv.style.marginTop = '12px';
                tempDiv.appendChild(textDiv);

                document.body.appendChild(tempDiv);
                // إخفاء باقي عناصر الصفحة مؤقتاً
                allEls.forEach(el => {
                  if (el !== tempDiv) {
                ...
                // تحويل العنصر لصورة
                window.html2canvas(tempDiv, {backgroundColor: '#fff', useCORS: true, logging: false}).then(canvas => {
                  // إعادة إظهار العناصر
                  ...
                  toast.textContent = 'تم تنزيل القصة كصورة، يمكنك الآن مشاركتها عبر أنستاجرام';
                  toast.style.position = 'fixed';
                  toast.style.bottom = '32px';
                  toast.style.right = '32px';
                  toast.style.background = '#F59E42';
                  ...
                  // إعادة إظهار العناصر في حال الخطأ
                  hiddenEls.forEach(el => { el.style.display = ''; });
                  document.body.removeChild(tempDiv);
                  alert('حدث خطأ أثناء إنشاء الصورة. جرب إزالة التدرجات أو الصور الخارجية.');
                });
              });
              shareWa && shareWa.addEventListener('click', () => {
                const storyText = this.storySteps && this.storySteps.length > 0 ? this.storySteps.join('\n\n') : '';
                const waText = encodeURIComponent(storyText);
    ...
    async generateCompleteStoryImages() {
        ...
        const unifiedPrompt = `
        أنشئ صورة واحدة مُركبة عالية الدقّة مكوّنة من ٤ مربعات متجاورة (شبكة 2x2 أو صف واحد 1x4 حسب نسبة العرض)، كل مربع يمثل مشهدًا متسلسلًا من القصة. التزم بالتوجيهات التالية حرفيًا:
        
        [القواعد العامة]
        - الأسلوب: رسم كرتوني (colorful cartoon) نظيف وملوّن، تفاصيل وجه واضحة، تعابير عاطفية معبرة.
        - الاتساق: احفظ ملامح الشخصية الرئيسية ثابتة عبر كل المربعات (نفس وجه، نفس لون العينين، نفس تسريحة الشعر وملابسها) — seed للشخصية: ${characterSeed}.
        - لوحة ألوان واحدة متناسقة عبر الأجزاء الأربعة.
        - لا تضف أي نص أو شعارات أو توقيعات أو علامات مائية داخل الصور. الاستثناء الوحيد: أرقام المشاهد العربية فقط (١، ٢، ٣، ٤) إن رغبت، توضع فنياً في زاوية كل مربع.
        - إخراج عالي الدقة (مثال مقترح: الناتج المركب 4096x4096 أو ما يعادل دقة كل جزء 2048x2048) بصيغة PNG أو WEBP.
        
        [التكوين والانتقال بين الأجزاء]
        - اعتبر الأربعة أجزاء كمشاهد متتابعة: حافظ على خلفيات/عناصر مرجعية مشتركة (ألوان/رموز/عناصر خلفية) لتوضيح التسلسل الزمني والمكاني.
        - تنويع قليل في زوايا الكاميرا (لقطة مقربة، متوسطة، واسعة) مع الحفاظ على منظور متناسق للشخصية.
        - مصدر إضاءة منطقي ومتسلسل بين المشاهد (أو إضاءة ثابتة إذا يناسب النص).
        
        [تفاصيل كل مربع — نفّذ بدقة]
        ١. ${storySteps[0]}
        - وصف المشهد: المكان والعنصر المركزي وحالة الشخصية الرئيسية (${this.heroName}).
        - شعور المشهد: (بداية/فضول/تحمّس).
        - ملاحظة: الشخصية تظهر بوضوح في المقدمة مع تعابير وجه مناسبة.
        
        ٢. ${storySteps[1]}
        - وصف المشهد: تطور/مواجهة أو دخول شخصية ثانوية.
        - شعور المشهد: (دهشة/صعوبة).
        - ملاحظة: احتفظ بنفس زي الشخصية؛ أضف عنصر بصري يربط بالمشهد الأول.
        
        ٣. ${storySteps[2]}
        - وصف المشهد: ذروة الصراع أو قرار حاسم.
        - شعور المشهد: (توتر/شجاعة).
        - ملاحظة: وضع ديناميكي وحركة واضحة، تعابير وجه قوية.
        
        ٤. ${storySteps[3]}
        - وصف المشهد: خاتمة أو حل أو لحظة احتفال/تأمل.
        - شعور المشهد: (ارتياح/فرح/حكمة).
        - ملاحظة: إضاءة أدفأ أو عناصر نجاح بصرية لتمييز النهاية.
        
        في كل مربع، حدد ما إذا كانت الشخصية الرئيسية ${this.heroName} تظهر بمفردها أو مع عناصر/شخصيات أخرى، ووصف قصير لملابسها وتعابير وجهها. (مثال: "الشخصية ${this.heroName} في مركز الإطار، ترتدي معطف أزرق قصير، تعابير: مصمم/مندهش").
        
        [قواعد منع الأخطاء — ضعها كـ negative prompt]
        - لا نصوص بأي لغة داخل الصورة (باستثناء أرقام المشاهد العربية إن سمحت).
        - لا شعارات/تواقيع/علامات مائية.
        - لا عناصر حديثة أو غير مناسبة للقصة ما لم تُذكر صراحة (لا هواتف ذكية، لا علامات تجارية).
        - تجنّب الواقعية المفرطة إذا المطلوب كرتوني — لا صور فوتورية.
        - لا عنف فاضح، لا محتوى جنسي أو غير لائق.
        - لا تشوهات في الوجوه أو تشوهات تشريحية، ولا تغيّر جذري في ملامح الشخصية بين المربعات.
        
        [التفاصيل المرئية والأسلوب]
        - وجوه معبرة، حواجب واضحة، ظلال ناعمة، وملمس خط ناعم.
        - خلفيات بسيطة داعمة لا تطغى على الشخصية.
        - عناصر زخرفية معتدلة (أوراق طيران، شرارات، شعاع ضوء عند النهاية).
        - إطار رفيع أو فاصل بسيط موحّد اللون بين المربعات (تانٍ أو ظل خفيف).
        
        [الإخراج والميتاداتا]
        - إخراج مركب واحد يجمع الأجزاء الأربعة دون فراغات كبيرة.
        - صيغة: PNG أو WEBP، دقة عالية.
        - اسم ملف مقترح: fullstory_${this.heroName}_${characterSeed}.png
        
        [خلاصة سريعة للإرسال للمولد]
        - style: "colorful cartoon", consistent character seed: ${characterSeed}, no text except Arabic numerals ١-٤ if needed, high-res composite PNG.
        - panels: 
          ١. ${storySteps[0]}
          ٢. ${storySteps[1]}
          ٣. ${storySteps[2]}
        ...
        اتبع هذه التعليمات بدقة لتضمن أن الصورة الناتجة متناسقة وملائمة لسرد القصة بالكامل.
        `;
